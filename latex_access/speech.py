...
denominators=[" sobre cero"," sobre uno "," mitad"," tercio"," cuarto"," quinto"," sexto"," séptimo"," octavo"," noveno"]
...
class speech(latex_access.translator):
    ...
    def super(self,input,start):
        ...
        #Handle primes
            ...
            translation = " a la  %s  " % self.translate(arg[0])  
    ...
    def sqrt(self,input,start):
        ...
        if opt_arg:
            ...
            if opt_arg[0]=="2": opt="cudrado"
            elif opt_arg[0]=="3": opt="cubo"
            elif opt_arg[0]=="": opt=""
            else: opt=opt_arg[0]+"ésima"
        else:
            arg=get_arg(input,start)
            opt=""
        if arg[0].isdigit() or len(arg[0])==1:
            translation="%s raíz %s" %(opt,arg[0])
        else:
            translation=" empieza %s raíz %s termina raíz" % (opt,self.translate(arg[0]))
        return (translation,arg[1])
    ...
    def frac(self,input,start):
        ...
        if len(numerator[0])==1 and len(denominator[0])==1:
            if numerator[0].isdigit() and denominator[0].isdigit():
                ...
                translation =" %s sobre %s " % (numerator[0], denominator[0])
        else:
            translation=" empieza fracción %s sobre %s termina fracción " % (self.translate(numerator[0]), self.translate(denominator[0]))
    ...
    def integral(self,input,start):
        ...
        if lower is not None and upper is not None and len(lower[0]) !=0 and len(upper[0]) !=0:
            output+="desde %s hasta %s de " % (self.translate(lower[0]),self.translate(upper[0]))
        elif lower is not None and len(lower[0]) !=0:
            output +="de %s" % self.translate(lower[0])
        return (output,i)
    ...
    def tag(self,input,start):
        ...
        arg=get_arg(input,start)
        translation=" tag parentesis izquierdo "+arg[0]+" parentesis derecho "
        return (translation,arg[1])
        ...
        if ';' in arg[0]: # we have mins possibly seconds too
            for x in arg[0]:
                if ';' == x: # we have mins/sec
                    counter=counter+1
                    if counter == 1:
                        translation=translation+" grados "
                        continue
                    elif counter == 2:
                        translation=translation+" minutos "
                        continue
                    elif counter == 3:
                        translation=translation+" segundos "
                        continue 
