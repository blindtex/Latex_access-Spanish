...
class translator:
    ...
    def translate(self,input,rting=()):        
        ...
        Returns string.'''
        if self.depth==0:
            if hasattr (self, "before"): #D.If the object has the attribute "before", how "before" is created?
                self.before ()
            self.rt=[] #Routing table. D. It creates a empty array.
            rting=(0,0)
            backupPosition = 0
            rtlen = 0
        ...
        consumed=0
        expandUntil = -1
        output=""#D. The final output, by the moment is empty.
        i=0
        while True:
